...
def estimate_worker_3d_position(image, model, num_classes, num_keypoints, cameraMatrix, distCoeffs):
    ...
    for detection in detections:
        # Estime a posição 3D do trabalhador a partir da detecção
...
def estimate_3d_position_from_detection(detection, cameraMatrix, distCoeffs):
    ...
    """
    #implemente aqui o algoritmo para estimar a posição 3D do trabalhador a partir da detecção
    #voce pode usar a função pnp() para obter a posição 3D a partir dos pontos 2D e 3D
