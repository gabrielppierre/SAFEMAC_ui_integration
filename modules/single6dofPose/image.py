...
def load_data_detection(imgpath, labpath, maskpath, shape, jitter, hue, saturation, exposure, bgpath, num_keypoints, max_num_gt):
...
def augment_without_mask(imagepath, labelpath, shape, jitter, hue, saturation, exposure, num_keypoints, max_num_gt):
    image = Image.open(imagepath).convert('RGB')
    image, flip, dx, dy, sx, sy = data_augmentation(image, shape, jitter, hue, saturation, exposure)
    ow, oh = image.size
    label = fill_truth_detection(labelpath, ow, oh, flip, dx, dy, 1./sx, 1./sy, num_keypoints, max_num_gt)
    return image, label

def augment_without_mask_2(imagepath, shape, jitter, hue, saturation, exposure, return_cv2=True):
    image = Image.open(imagepath).convert('RGB')
    image, flip, dx, dy, sx, sy = data_augmentation(image, shape, jitter, hue, saturation, exposure)
