...
def train(epoch, savedir):
    ...
    train_loader = torch.utils.data.DataLoader(dataset.listDataset(trainlist, train_percentages,
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss, conf = region_loss(output, target, epoch)
        ...
        optimizer.step()
        t9 = time.time()
        with open(os.path.join(savedir, 'confs.txt'), 'a') as conffile:
            conffile.write('{}\n'.format(conf))
...
def get_arguments():
    ...
    parser.add_argument('--ymlcfg',  type=str, required=True)          # Path to the file with the training configuration for the object trained
    parser.add_argument('--srcpath', type=str, default='./../rsc')        # Parent folder where the resources are contained
...
if __name__ == "__main__":
    ...
    # transform the paths on the yml to abs path if the parameters abs_path is False
    # use the path of args.srcpath as the base directory
    if not data_config['abs_path']:
        ...
        data_config['initweights'] = os.path.join(args.srcpath, data_config['initweights'])
        data_config['trainpaths']  = [os.path.join(args.srcpath, x) for x in data_config['trainpaths']] 
        data_config['testpaths']   = [os.path.join(args.srcpath, x) for x in data_config['testpaths']]
        
    initweightfile = data_config['initweights']

    modelcfg      = data_config['modelcfg']
    net_options   = parse_cfg(modelcfg)[0]   # Load the network parameters from yolo-pose.cfg
    
    trainlist           = data_config['trainpaths']       # Get the list of folders containing the images to train
    train_percentages   = data_config['trainpercentages'] # Get the percentages of images to use in each folder
    testlist            = data_config['testpaths']        # Get the list of folders containing the images to test
    test_percentages    = data_config['testpercentages']  # Get the percentages of images to use in each folder
    
    gpus          = data_config['gpus'] # Load the GPU id
    meshname      = data_config['mesh'] # Load the CAD model
    num_workers   = int(data_config['num_workers']) 
    backupdir     = data_config['backup'] # Load the path to save the output files
    
    vx_threshold  = float(data_config['diam']) * 0.1 # threshold for the ADD metric
    
    if not os.path.exists(backupdir):
        makedirs(backupdir)
    
    num_images = 0
    for idx, datapath in enumerate(trainlist):
        dataimages = os.listdir(datapath)
        num_images += int(len(dataimages)*train_percentages[idx])
    
    # Load the training parameters from the yolo-pose.cfg
    ...
    bg_file_names = get_all_files(data_config['backimages'])
    ...
    # Get the dataloader for test and train data
    test_loader = torch.utils.data.DataLoader(dataset.listDataset(testlist, test_percentages,
