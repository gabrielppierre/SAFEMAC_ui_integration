...
def draw_mesh(image, vertices2d, indices):
    ...
    for index in indices:
        triangle = np.array(
            [vertices2d[index[0]], vertices2d[index[1]], vertices2d[index[2]]], dtype='int32')
...
def project_mesh(cadModel, bounds, cameraMatrix, frame, coefficients=np.zeros((1,5)), pose=None):
    ...
    #printed1 = axys_image1
    cv2.imshow('Projected Mesh CV2', printed1)

    cv2.imshow('Original', frame)
    
    printed2 = draw_corners(frame, bounds, text=True)
...
if __name__ == '__main__':
    ...
    CADPATH = F + '\\rsc2\\data\\urna2\\urna_rotated180_2.ply'
    DATASET = F + '\\rsc2\\data\\urna2\\train'
    #DATASET = F + '\\..\\rsc\\data\\TEST/EOS\\fast/car'
    #DATASET = F + '\\..\\rsc\\data\\TEST/EOS\\scale/squirrel'
    #DATASET = F + '\\..\\rsc\\data\\UNITY\\train/squirrel'

    unity = False
    unity2 = False
    
    CAMERAVALUES = F + '\\..\\..\\rsc\\calibration/X\\camera_matrix.txt'
    COEFFICIENTS = F + '\\..\\..\\rsc\\calibration/X\\coefficients.txt'
    ...
    for name in images:
        ...
        values = values[3:-2].reshape((8,2))
        values[:, 0] = values[:, 0] * image.shape[1]
        values[:, 1] = values[:, 1] * image.shape[0]
        # values = np.array(values, dtype=np.int32)

        if unity: # Rotation to bunny and squirrel
            valuestmp = np.zeros((8,2))
            valuestmp[0] = values[0]
            valuestmp[1] = values[2]
            valuestmp[2] = values[1]
            valuestmp[3] = values[3]
            valuestmp[4] = values[4]
            valuestmp[5] = values[6]
            valuestmp[6] = values[5]
            valuestmp[7] = values[7]
            values = valuestmp
            valuestmp = np.zeros((8,2))
            valuestmp[0] = values[4]
            valuestmp[4] = values[0]
            valuestmp[1] = values[5]
            valuestmp[5] = values[1]
            valuestmp[6] = values[2]
            valuestmp[2] = values[6]
            valuestmp[7] = values[3]
            valuestmp[3] = values[7]
            values = valuestmp

        if unity2: # Rotation to car
            valuestmp = np.zeros((8,2))
            valuestmp[0] = values[2]
            valuestmp[1] = values[0]
            valuestmp[3] = values[1]
            valuestmp[2] = values[3]
            valuestmp[5] = values[4]
            valuestmp[4] = values[6]
            valuestmp[6] = values[7]
            valuestmp[7] = values[5]
            values = valuestmp

    
        camera = np.array([[500., 0., 300.],[0., 500., 300.],[0., 0., 1.]])#np.loadtxt(CAMERAVALUES).reshape((3,3))
        camera[0,:] = camera[0,:] * (image.shape[1]/W)
        camera[1,:] = camera[1,:] * (image.shape[0]/H)
    
        coefficients = np.zeros((1,8)) #np.loadtxt(COEFFICIENTS).reshape((1,-1))

        if POSEFOLDER:
            pose = np.loadtxt(imagePath.replace(IMGFOLDER, POSEFOLDER).replace('jpg', 'txt'))
        else:
            pose = None
